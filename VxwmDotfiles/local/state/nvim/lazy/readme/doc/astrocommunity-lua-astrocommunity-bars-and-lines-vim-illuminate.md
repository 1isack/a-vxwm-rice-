# vim-illuminate

> **Note:** As of AstroNvim v6, word highlighting is built in via the `snacks.words` module in `folke/snacks.nvim`. This plugin is still available for users who prefer it.

(Neo)Vim plugin for automatically highlighting other uses of the word under the cursor using either LSP, Tree-sitter, or regex matching.

**Repository:** <https://github.com/RRethy/vim-illuminate>

<!-- vim: set ft=markdown: -->